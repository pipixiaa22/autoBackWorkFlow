...
public class AnalysisApplicationService {
    ...
    public AnalysisApplicationService(ProjectApplicationService projects, SkillApplicationService skills, AdsAnalysisRunService runs,
                                      AdsDialogueSegmentService segments, LlmProvider llm, ObjectMapper mapper) {
        this.projects=projects; this.skills=skills; this.runs=runs; this.segments=segments; this.llm=llm; this.mapper=mapper;
    }
    @Transactional public AdsAnalysisRun create(Long projectId, AnalysisDtos.CreateAnalysisRequest request) {
        ...
        AdsAnalysisRun run = new AdsAnalysisRun(); run.setRunNo("AN-"+UUID.randomUUID().toString().replace("-", "").substring(0, 12).toUpperCase()); run.setProjectId(projectId); run.setProjectVersion(project.getVersion()); run.setSkillVersionId(skill.getId()); run.setProviderId(request.providerId()); run.setModelId(request.modelId()); run.setProviderCodeSnapshot(llm.providerCode()); run.setRewriteMode(request.rewriteMode() == null ? "STRICT" : request.rewriteMode()); run.setBackgroundSnapshot(project.getStoryBackground()); run.setDialogueSnapshot(project.getOriginalDialogue()); run.setInputHash(Hashing.sha256((project.getStoryBackground()==null?"":project.getStoryBackground())+"\n"+project.getOriginalDialogue())); run.setPromptHash(Hashing.sha256(skill.getSystemPromptTemplate())); run.setModelParametersJson(skill.getModelParametersJson()); run.setStatus("RUNNING"); run.setVersion(1); run.setCreatedAt(now); run.setUpdatedAt(now); run.setStartedAt(now); runs.save(run);
        try {
            AnalysisCandidate candidate = llm.analyse(new LlmProvider.LlmAnalysisCommand(project.getStoryBackground(), project.getOriginalDialogue(), run.getRewriteMode(), skill.getSystemPromptTemplate()));
