...
public class DialogueApplicationService {
    ...
    public DialogueApplicationService(AdsDialogueSegmentService segments, AdsAudioAssetService assets,
        ...
        this.segments = segments; this.assets = assets; this.projects = projects;
    }
    ...
    public AdsDialogueSegment update(Long segmentId, SegmentDtos.UpdateSegmentRequest request) {
        ...
        if (!segments.update(update)) throw BizException.conflict("DIALOGUE_VERSION_CONFLICT", "分段已被其他修改覆盖，请刷新后重试");
    ...
    public List<AdsDialogueSegment> split(Long segmentId, SegmentDtos.SplitSegmentRequest request) {
        ...
        if (request.splitOffset() >= text.length()) throw BizException.badRequest("DIALOGUE_INVALID_SPLIT", "拆分位置必须在原始台词内部");
        String left = text.substring(0, request.splitOffset()); String right = text.substring(request.splitOffset());
        if (left.isBlank() || right.isBlank()) throw BizException.badRequest("DIALOGUE_INVALID_SPLIT", "拆分后不能出现空白分段");
        ...
        segments.save(second); markAssetsStale(segmentId);
        return list(first.getProjectId());
    }

    @Transactional
    public List<AdsDialogueSegment> merge(SegmentDtos.MergeSegmentsRequest request) {
        AdsDialogueSegment first = required(request.firstSegmentId()); AdsDialogueSegment second = required(request.secondSegmentId());
        ...
        markAssetsStale(first.getId()); markAssetsStale(second.getId()); return list(first.getProjectId());
    }

    @Transactional
    public List<AdsDialogueSegment> reorder(Long projectId, SegmentDtos.ReorderSegmentsRequest request) {
        AdsProject project = projects.required(projectId); verifyVersion(project.getVersion(), request.projectVersion(), "PROJECT_VERSION_CONFLICT");
        ...
        for (int i = 0; i < request.segmentIds().size(); i++) segments.update(Wrappers.<AdsDialogueSegment>lambdaUpdate().eq(AdsDialogueSegment::getId, request.segmentIds().get(i)).set(AdsDialogueSegment::getSegmentNo, i + 1));
    ...
    public AdsDialogueSegment required(Long id) {
        ...
        if (result == null) throw BizException.notFound("DIALOGUE_NOT_FOUND", "台词分段不存在"); return result;
    }
    private void markAssetsStale(Long segmentId) { assets.update(Wrappers.<AdsAudioAsset>lambdaUpdate().eq(AdsAudioAsset::getSegmentId, segmentId).eq(AdsAudioAsset::getDeleted, 0).set(AdsAudioAsset::getStatus, "STALE")); }
    private static void verifyVersion(Integer actual, Integer wanted, String code) { if (actual == null || !actual.equals(wanted)) throw BizException.conflict(code, "资源已被其他修改覆盖，请刷新后重试"); }
    private static boolean safeEquals(Object a, Object b) { return a == null ? b == null : a.equals(b); }
    private static void copyForSplit(AdsDialogueSegment source, AdsDialogueSegment target, String text, int no, int start, Integer end, Date now) {
        target.setProjectId(source.getProjectId()); target.setSegmentNo(no); target.setSpeakerId(source.getSpeakerId()); target.setSpeakerNameSnapshot(source.getSpeakerNameSnapshot()); target.setSemanticGroup(source.getSemanticGroup()); target.setSourceStart(start); target.setSourceEnd(end); target.setOriginalText(text); target.setSpokenText(text); target.setSubtitleText(text); target.setEmotionJson(source.getEmotionJson()); target.setToneJson(source.getToneJson()); target.setSpeed(source.getSpeed()); target.setVolume(source.getVolume()); target.setPauseBeforeMs(0); target.setPauseAfterMs(source.getPauseAfterMs()); target.setEmphasisJson(source.getEmphasisJson()); target.setVoiceDirection(source.getVoiceDirection()); target.setRewriteMode(source.getRewriteMode()); target.setRewriteLevel("MANUAL"); target.setManualEdited(1); target.setAudioStale(1); target.setVersion(1); target.setCreatedAt(now); target.setUpdatedAt(now); target.setDeleted(0);
    }
}
