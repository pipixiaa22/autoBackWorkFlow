...
public class AudioController {
    ...
    @PostMapping(value="/projects/{projectId}/audio-reference-assets",consumes=MediaType.MULTIPART_FORM_DATA_VALUE) public ApiResponse<AdsAudioAsset> uploadReferenceAudio(@PathVariable Long projectId,@RequestPart("file") org.springframework.web.multipart.MultipartFile file){return ApiResponse.ok(audio.uploadReferenceAudio(projectId,file));}
    ...
    @GetMapping("/projects/{projectId}/audio-tasks") public ApiResponse<com.ckrey.autobackworkflow.common.api.CursorPage<AdsGenerationTask>> list(@PathVariable Long projectId,@RequestParam(required=false) String cursor,@RequestParam(required=false) Integer limit){return ApiResponse.ok(audio.list(projectId,cursor,limit));}
    ...
    @GetMapping("/audio-assets/{id}") public ApiResponse<AdsAudioAsset> asset(@PathVariable Long id){return ApiResponse.ok(audio.asset(id));}
    @GetMapping("/audio-assets/{id}/download") public ResponseEntity<Resource> downloadAsset(@PathVariable Long id){
        AdsAudioAsset asset=audio.asset(id);
        MediaType type;
        try { type=asset.getMediaType()==null?MediaType.APPLICATION_OCTET_STREAM:MediaType.parseMediaType(asset.getMediaType()); }
